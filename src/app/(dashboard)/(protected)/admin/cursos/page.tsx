...
import { CourseCoordinadoresModal } from "@/components/admin/CourseCoordinadoresModal";
...
export default async function AdminCursosPage() {
  ...
  const [{ data: courses }, { data: careers }, { data: docentes }, { data: coordinadores }] = await Promise.all([
    ...
    supabase.from("users").select("id, nombre, apellido").eq("role", "coordinador"),
  ]);

  const careerOptions = careers ?? [];
  const docenteOptions = docentes ?? [];
  const coordinadorOptions = coordinadores ?? [];
  ...
  const { data: courseCoordinadorRows } = courseIds.length
    ? await supabase.from("course_coordinadores").select("course_id, coordinador_id").in("course_id", courseIds)
    : { data: [] as { course_id: string; coordinador_id: string }[] };

  const coordinadorIdsByCourse = new Map<string, string[]>();
  for (const row of courseCoordinadorRows ?? []) {
    const list = coordinadorIdsByCourse.get(row.course_id as string) ?? [];
    list.push(row.coordinador_id as string);
    coordinadorIdsByCourse.set(row.course_id as string, list);
  }
                ...
                <div className="flex items-center gap-1.5">
                  {course.estado === "revision" ? (
                    ...
                    <>
                      ...
                      <CourseCoordinadoresModal
                        courseId={course.id}
                        courseTitulo={course.titulo}
                        coordinadores={coordinadorOptions}
                        assignedIds={coordinadorIdsByCourse.get(course.id) ?? []}
                      />
