...
import { TallerInscriptosModal } from "@/components/admin/TallerInscriptosModal";
...
export default async function AdminTalleresPage() {
  ...
  const tallerIds = (talleres ?? []).map((t) => t.id as string);
  const { data: inscripciones } = tallerIds.length
    ? await supabase
        .from("taller_inscripciones")
        .select("id, taller_id, asistio, users(nombre, apellido)")
        .in("taller_id", tallerIds)
    : { data: [] as Record<string, unknown>[] };

  const inscriptosByTaller = new Map<string, { id: string; nombre: string; apellido: string; asistio: boolean }[]>();
  for (const row of inscripciones ?? []) {
    const list = inscriptosByTaller.get(row.taller_id as string) ?? [];
    const u = row.users as unknown as { nombre: string; apellido: string } | null;
    list.push({
      id: row.id as string,
      nombre: u?.nombre ?? "—",
      apellido: u?.apellido ?? "",
      asistio: row.asistio as boolean,
    });
    inscriptosByTaller.set(row.taller_id as string, list);
  }
            ...
            <TableRow key={taller.id}>
              ...
              <TableCell>
                <div className="flex items-center gap-2">
                  ...
                  <TallerInscriptosModal
                    tallerTitulo={taller.titulo}
                    inscriptos={inscriptosByTaller.get(taller.id) ?? []}
                  />
