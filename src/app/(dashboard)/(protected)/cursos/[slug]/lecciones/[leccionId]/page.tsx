  ...
  const attachments: LessonAttachmentLink[] = (
    await Promise.all(
      (attachmentRows ?? []).map(async (row) => {
        const url = await getSignedLessonContentUrl(supabase, row.archivo_url as string);
        return url ? { id: row.id as string, titulo: row.titulo as string, url } : null;
      })
    )
  ).filter((a): a is LessonAttachmentLink => a !== null);
